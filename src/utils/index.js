import { v4 as uuidv4 } from 'uuid';

export const theOfficeCharacters = [
  {
    id: uuidv4(),
    favorito: false,
    valorNome: "Michael Scott",
    valorTime: "Gerente Regional",
    valorCargo:
      "O excêntrico e carismático gerente da Dunder Mifflin Scranton. Conhecido por seu humor inapropriado e desejo de ser amado por todos.",
    valorImg: "https://upload.wikimedia.org/wikipedia/en/d/dc/MichaelScott.png",
  },
  {
    id: uuidv4(),
    favorito: false,
    valorNome: "Dwight Schrute",
    valorTime: "Assistente do Gerente Regional",
    valorCargo:
      "Ele se considera o líder da equipe e frequentemente se autodenomina ''Assistente do Gerente Regional'', embora o título real seja bem menos impressionante. ",
    valorImg:
      "https://upload.wikimedia.org/wikipedia/en/c/cd/Dwight_Schrute.jpg",
  },
  {
    id: uuidv4(),
    favorito: false,
    valorNome: "Jim Halpert",
    valorTime: "Vendas",
    valorCargo:
      "Vendedor carismático e brincalhão. Tem uma relação complicada com Pam Beesly e adora pregar peças em Dwight.",
    valorImg: "https://upload.wikimedia.org/wikipedia/en/7/7e/Jim-halpert.jpg",
  },
  {
    id: uuidv4(),
    favorito: false,
    valorNome: "Stanley Hudson",
    valorTime: "Vendas",
    valorCargo:
      "Vendedor veterano, sempre entediado e esperando pela aposentadoria. Conhecido por seu amor por palavras cruzadas e desprezo por reuniões.",
    valorImg:
      "https://th.bing.com/th/id/R.8fda9397e3e72d9f4817314328f02d31?rik=6f6ggCemqeYUjg&riu=http%3a%2f%2fstatic1.wikia.nocookie.net%2f__cb20090312202638%2ftheoffice%2fimages%2f7%2f73%2f2009StanleyCropped.PNG&ehk=YaGMqJF6jzuewrl6RHZrVArQc9MtQkuInox3i6zN9CE%3d&risl=&pid=ImgRaw&r=0",
  },
  {
    id: uuidv4(),
    favorito: false,
    valorNome: "Phyllis Vance",
    valorTime: "Vendas",
    valorCargo:
      "Vendedora doce, mas perspicaz, que frequentemente surpreende seus colegas com seu sarcasmo.",
    valorImg:
      "https://th.bing.com/th/id/R.d502373c236eca1e260b27132f84fed5?rik=ctJCoQbceblhnw&riu=http%3a%2f%2fimages2.fanpop.com%2fimages%2fphotos%2f4800000%2fPhyllis-New-Promo-Photo-the-office-4836142-1535-2048.jpg&ehk=bsHd0B7Mv62OtBb3YHJnj9%2bk9vmGPR0%2fAGLOn3bzN70%3d&risl=&pid=ImgRaw&r=0",
  },
  {
    id: uuidv4(),
    favorito: false,
    valorNome: "Andy Bernard",
    valorTime: "Vendas",
    valorCargo:
      "Ex-aluno de Cornell, ansioso por aprovação e amante de a cappella. Posteriormente se torna gerente da filial.",
    valorImg: "https://th.bing.com/th/id/OIP.3JGn0bj4Ovaj7b7gFn7jRwHaJ4?rs=1&pid=ImgDetMain",
  },
  {
    id: uuidv4(),
    favorito: false,
    valorNome: "Pam Beesly",
    valorTime: "Recepção",
    valorCargo:
      "Recepcionista gentil e criativa, que sonha em ser artista. Eventualmente, se torna vendedora.",
    valorImg: "https://upload.wikimedia.org/wikipedia/en/6/67/Pam_Beesley.jpg",
  },
  {
    id: uuidv4(),
    favorito: false,
    valorNome: "Angela Martin",
    valorTime: "Contabilidade",
    valorCargo:
      "Contadora rígida, amante de gatos e profundamente religiosa. Frequentemente julgadora e sarcástica.",
    valorImg:
      "https://upload.wikimedia.org/wikipedia/en/0/0b/Angela_Martin.jpg",
  },
  {
    id: uuidv4(),
    favorito: false,
    valorNome: "Kevin Malone",
    valorTime: "Contabilidade",
    valorCargo:
      "Contador de poucas palavras e mente simples, mas com um grande amor por comida.",
    valorImg:
      "https://img.nbc.com/sites/nbcunbc/files/metaverse_assets/1/0/6/3/3/6/kevin-500x500.jpg?impolicy=nbc_com&imwidth=480&imdensity=1",
  },
  {
    id: uuidv4(),
    favorito: false,
    valorNome: "Oscar Martinez",
    valorTime: "Contabilidade",
    valorCargo:
      "Contador mais sério e inteligente do escritório. Conhecido por sua paciência e sarcasmo.",
    valorImg:
      "https://th.bing.com/th/id/OIP.piUl6FVfUj_C7JNqWGW-iwHaJ4?rs=1&pid=ImgDetMain",
  },
  {
    id: uuidv4(),
    favorito: false,
    valorNome: "Darryl Philbin",
    valorTime: "Armazém",
    valorCargo:
      "Trabalhador do armazém, prático e sábio. Eventualmente, sobe na empresa.",
    valorImg:
      "https://th.bing.com/th/id/OIP.4ZddcGlpHlOU74ofWKi2sgHaJ4?rs=1&pid=ImgDetMain",
  },
  {
    id: uuidv4(),
    favorito: false,
    valorNome: "Roy Anderson",
    valorTime: "Armazém",
    valorCargo:
      "Funcionário do armazém. Ciumento e pouco ambicioso.",
    valorImg: "https://th.bing.com/th/id/R.92fff8debd953b285121368a8b9dd3a6?rik=jWziAjM0TTIPyg&pid=ImgRaw&r=0",
  },
  {
    id: uuidv4(),
    favorito: false,
    valorNome: "Madge Madsen",
    valorTime: "Armazém",
    valorCargo:
      "Trabalhadora do armazém. Pouco fala, mas é respeitada por sua força.",
    valorImg:
      "https://th.bing.com/th/id/R.b152d1d879962476a86b01ee9ca187ea?rik=enVhDEDaOtYKNA&riu=http%3a%2f%2fvignette4.wikia.nocookie.net%2ftheoffice%2fimages%2f0%2f0e%2fMadge.jpg%2frevision%2flatest%3fcb%3d20060528224950&ehk=r7v4ZjHfxZvDSH7DLD446fDNOoWm%2fA22y4HHscYJ4tc%3d&risl=&pid=ImgRaw&r=0",
  },
  {
    id: uuidv4(),
    favorito: false,
    valorNome: "Lonny Collins",
    valorTime: "Armazém",
    valorCargo:
      "Trabalhador do armazém, conhecido por sua atitude sarcástica e brincalhona.",
    valorImg: "https://th.bing.com/th/id/OIP.gOYQWVrzH3fkLgiA56sntQHaHD?rs=1&pid=ImgDetMain",
  },
];

